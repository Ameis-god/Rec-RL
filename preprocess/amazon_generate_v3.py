    ...
    for split, file in zip(['train', 'valid'], [args.train_top_file, args.test_top_file]):
        ...
        with open(file, 'r') as f:
            for line in f:
                ...
                history = '; '.join([meta_infos[x] for x in history])
                
                uid2next_template = random.choice(prompt_templates['uid2next'])
                # TODO: Change top1 to iid
                uid2next_behavior.append({'uid': uid, 'iid': iid, 'question': history, 'answer': meta_infos[top1], 'template': uid2next_template, 'type': 'uid2next_t'})
                uid2next_both.append({'uid': uid, 'iid': iid, 'question': '<user>'+history, 'answer': meta_infos[top1], 'template': uid2next_template, 'type': 'uid2next'})
                
                # uid2next_intention.append({'uid': uid, 'iid': iid, 'question': '<user>', 'answer': meta_infos[iid]['title'], 'template': uid2next_template, 'type': 'uid2next'})
                # uid2next_behavior.append({'uid': uid, 'iid': iid, 'question': history, 'answer': meta_infos[iid]['title'], 'template': uid2next_template, 'type': 'uid2next_t'})
                # uid2next_both.append({'uid': uid, 'iid': iid, 'question': '<user>'+history, 'answer': meta_infos[iid]['title'], 'template': uid2next_template, 'type': 'uid2next'})
        ...
        if split=='train':
            ...
            # add
            train_onlynext.extend(uid2next_behavior)
            train_onlynext.extend(uid2next_both)
        elif split=='valid':
            ...
            # add
            valid_onlynext.extend(uid2next_behavior)
            valid_onlynext.extend(uid2next_both)
...
if __name__ == '__main__':
    ...
    if args.gpt_query_file is not None:
        ...
        train_sharegpt, valid_sharegpt = gen_sharegpt(args, tokenizer, 10000, 1000)
        ...
        train_onlynext.extend(train_sharegpt)
        valid_onlynext.extend(valid_sharegpt)
        ...
        print(f"train_onlynext: {len(train_onlynext)}, valid_onlynext: {len(valid_onlynext)}")
        ...
        json.dump(train_onlynext, open(args.save_both_file+'_train_onlynext.json', 'w'))
        json.dump(valid_onlynext, open(args.save_both_file+'_valid_onlynext.json', 'w'))