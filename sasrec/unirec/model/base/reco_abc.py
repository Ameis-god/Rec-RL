...
class AbstractRecommender(nn.Module):    
    ...
    def _parameter_validity_check(self):
        ...
        if self.loss_type == LossFuncType.SOFTMAX.value:            
            if self.config['train_file_format'] in [DataFileFormat.T2.value, DataFileFormat.T2_1.value] and self.group_size <= 0:
                raise ValueError(r''' 
    ...
    def _init_modules(self):
        ...
        if self.use_pre_item_emb and 'item_emb_path' in self.config: #check for item_emb_path is to ensure that in infer/test task (or other cases that need to load model ckpt), we don't load pre_item_emb which will be overwritten by model ckpt.
            pre_item_emb = file_io.load_pre_item_emb(self.config['item_emb_path'], self.logger) 
        ...
        if self.use_text_emb and 'text_emb_path' in self.config:
            text_emb = file_io.load_pre_item_emb(self.config['text_emb_path'], self.logger)
    ...
    def _init_params(self):
        ...
        # Initialize item_embedding weights to zero at the end
        nn.init.constant_(self.item_embedding.weight.data, 0)
