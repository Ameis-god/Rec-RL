...
class WeightedLossTrainer(Trainer):
    """
    Custom Trainer class that applies different weights to losses based on task type
    """
    def __init__(self, task_weights=None, *args, **kwargs):
        ...
        self.task_weights = task_weights or {
            ...
            "uid2next": 1.0,
            "sharegpt": 1.0,
            "default": 1.0  # Default weight for other task types
        }
        self._signature_columns = [
            "input_ids", "attention_mask", "user_pos", "user_ids", 
            "item_seq", "item_pos", "item_ids", "labels", "type"
        ]
        # Add a counter to limit print times
        self.print_count = 0
        self.max_prints = 5  # Only print the first 5 batches
    
    def compute_loss(self, model, inputs, return_outputs=False):
        """
        Calculate weighted loss based on task type
        """
        # Get task type
        task_types = inputs.pop("type", None)
        
        # Forward pass
        outputs = model(**inputs)
        original_loss = outputs.loss
        
        # If task type is provided, apply weighted loss
        if task_types is not None:
            # Get individual loss for each sample (before reduction)
            labels = inputs.get("labels")
            logits = outputs.logits
            
            # Manually calculate loss for each sample
            ...
            # Sum over sequence length and normalize by number of non-padding tokens
            sequence_lengths = torch.clamp(padding_mask.sum(dim=1), min=1)  # Avoid division by zero
            loss_per_sample = (loss_per_token * padding_mask).sum(dim=1) / sequence_lengths
            
            # Apply weights based on task type and print information
            weighted_losses = []
            task_type_counts = {}
            
            for i, task in enumerate(task_types):
                weight = self.task_weights.get(task, self.task_weights["default"])
                weighted_losses.append(loss_per_sample[i] * weight)
                
                # Count the number of each task type
                if task not in task_type_counts:
                    task_type_counts[task] = 1
                else:
                    task_type_counts[task] += 1
            
            # Calculate the average of weighted losses
            weighted_loss = torch.stack(weighted_losses).mean()
            
            # Limit print times to avoid excessive logging
            if self.print_count < self.max_prints:
                print("\n" + "="*50)
                print(f"Batch {self.print_count + 1} Loss Information:")
                print(f"Original Loss: {original_loss.item():.4f}")
                print(f"Weighted Loss: {weighted_loss.item():.4f}")
                print(f"Task Types in Batch: {task_type_counts}")
                
                # Print loss and weight for each sample
                print("\nSample-level information:")
                for i, (task, loss_val) in enumerate(zip(task_types, loss_per_sample)):
                    weight = self.task_weights.get(task, self.task_weights["default"])
                    weighted_loss_val = loss_val.item() * weight
                    print(f"  Sample {i}: Task={task}, Loss={loss_val.item():.4f}, Weight={weight}, Weighted Loss={weighted_loss_val:.4f}")
                
                print("="*50 + "\n")
                self.print_count += 1
            
            # Replace original loss with weighted loss
...
def main():
    ...
    if training_args.do_eval:
        ...
        for data_type in ["iid2title", "sharegpt", "uid2hist", "uid2next", "uid2next_t", "uidiid2rank", "uidiid2binary"]:
    ...
    # Initialize our custom Trainer instead of the standard one
    ...
    if training_args.do_train:
        ...
        start_msg = (
            f"Start training model: {model_name}\n"
            f"Training samples: {len(train_dataset)}\n"
            f"Learning rate: {training_args.learning_rate}\n"
            f"Batch Size: {training_args.per_device_train_batch_size}\n"
            f"Training epochs: {training_args.num_train_epochs}"
        )
        logger.info(start_msg, "Training started")
        checkpoint = None
        if training_args.resume_from_checkpoint is not None:
            checkpoint = training_args.resume_from_checkpoint
            logger.info(f"Resume training from checkpoint: {checkpoint}", "Resume training")
        elif last_checkpoint is not None:
            checkpoint = last_checkpoint
            logger.info(f"Resume training from last checkpoint: {checkpoint}", "Resume training")
        ...
        train_msg = (
            ...
            f"Training loss: {metrics.get('train_loss', 'N/A')}\n"
            f"Training samples: {metrics['train_samples']}\n"
            f"Training duration: {metrics.get('train_runtime', 'N/A')} seconds"
        )
        logger.info(train_msg, "Training completed")

    # Evaluation
    if training_args.do_eval:
        logger.info("*** Evaluate ***")
        logger.info("Start evaluating model...", "Evaluation started")
        ...
        eval_msg = (
            ...
            f"Evaluation loss: {metrics.get('eval_all_loss', 'N/A')}\n"
            f"Perplexity: {perplexity}\n"
            f"Evaluation samples: {len(eval_dataset)}"
        )
        logger.info(eval_msg, "Evaluation completed")
