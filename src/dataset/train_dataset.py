...
class TrainingDataset4Exp(Dataset):
    def __init__(self, args, tokenizer, split='train', data_type_filter=None):
        ...
        self.function_map = {
            ...
            'uid2next_t': self.process_uid2hist,
    ...
    def load_user_history(self, file_path):
        """Load user history from tsv file with format: user_id\titem_id1,item_id2,..."""
        user_seq = {}
            ...
            for line in f:
                try:
                    ...
                    if parts[0] == "user_id":
                        continue
                    user_id = int(parts[0])
                    item_seq = [int(item_id) for item_id in parts[1].split(',')]
                    user_seq[user_id] = item_seq
